...
이 모듈은 Agent Cards MCP 서버를 구현합니다. 에이전트 카드 저장소를
관리하고 임베딩 기반 에이전트 검색 기능을 제공합니다.

주요 기능:
    - 에이전트 카드 로드 및 임베딩 생성
    - 자연어 쿼리로 에이전트 검색 (find_agent)
    - 여행 데이터 쿼리 (query_travel_data)
    - Google Places API 연동 (query_places_data)
    - 에이전트 카드 리소스 제공
...
데이터 소스:
    - agent_cards/: 에이전트 카드 JSON 파일 디렉토리
    - travel_agency.db: SQLite 여행 데이터베이스
"""
# type: ignore
import json
import os
...
def generate_embeddings(text):
    """Google Generative AI를 사용하여 텍스트 임베딩을 생성합니다.

    Args:
        text: 임베딩을 생성할 입력 문자열.

    Returns:
        list: 입력 텍스트를 나타내는 임베딩 리스트.
    """
...
def load_agent_cards():
    """지정된 디렉토리에서 에이전트 카드 JSON 파일을 로드합니다.

    Returns:
        tuple[list, list]: 카드 URI 목록과 에이전트 카드 데이터 목록.
        디렉토리가 비어있거나 JSON 파일이 없으면 빈 리스트를 반환합니다.
    """
    card_uris = []
    agent_cards = []
...
def build_agent_card_embeddings() -> pd.DataFrame:
    """에이전트 카드를 로드하고 임베딩을 생성하여 DataFrame으로 반환합니다.

    Returns:
        pd.DataFrame: 에이전트 카드와 임베딩이 포함된 DataFrame.
        카드가 없거나 오류 발생 시 None을 반환합니다.
    """
...
def serve(host, port, transport):  # noqa: PLR0915
    """Agent Cards MCP 서버를 초기화하고 실행합니다.

    Args:
        host: 서버 바인딩 호스트 주소.
        port: 서버 바인딩 포트 번호.
        transport: MCP 서버 전송 프로토콜 ('stdio', 'sse').

...
이 모듈은 여행 관련 에이전트들의 시스템 지시사항을 정의합니다.

포함된 프롬프트:
    - AIRFARE_COT_INSTRUCTIONS: 항공권 예약 에이전트
    - HOTELS_COT_INSTRUCTIONS: 호텔 예약 에이전트
    - CARS_COT_INSTRUCTIONS: 렌터카 예약 에이전트
    - PLANNER_COT_INSTRUCTIONS: 여행 계획 에이전트
    - SUMMARY_COT_INSTRUCTIONS: 여행 요약 생성
    - QA_COT_PROMPT: 컨텍스트 기반 질의응답

Note:
    모든 프롬프트는 영문으로 유지됩니다 (LLM 지시사항).
"""
# --- 항공권 예약 에이전트 시스템 지시사항 ---

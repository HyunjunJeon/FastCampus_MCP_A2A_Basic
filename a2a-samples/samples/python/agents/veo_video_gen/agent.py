...
비디오를 생성하는 에이전트를 구현합니다. 생성된 비디오는
GCS(Google Cloud Storage)에 저장되고 서명된 URL을 제공합니다.

주요 기능:
    - VEO 모델을 통한 텍스트→비디오 생성
    - 비동기 폴링으로 생성 진행률 추적
    - GCS에 비디오 저장 및 서명된 URL 생성
    - 안전 필터 처리
...
class VideoGenerationAgent:
    """VEO 모델을 사용하여 텍스트에서 비디오를 생성하는 에이전트.

    텍스트 프롬프트를 받아 VEO 모델로 비디오를 생성하고,
    GCS에 저장한 후 서명된 URL을 제공합니다. 진행률 업데이트를
    스트리밍으로 제공합니다.

    Attributes:
        SUPPORTED_INPUT_CONTENT_TYPES: 지원하는 입력 콘텐츠 타입.
        SUPPORTED_OUTPUT_CONTENT_TYPES: 지원하는 출력 콘텐츠 타입.
        VEO_MODEL_NAME: 사용할 VEO 모델 이름 (환경 변수로 설정 가능).
        VEO_POLLING_INTERVAL_SECONDS: 폴링 간격 (초).
        GCS_BUCKET_NAME_ENV_VAR: GCS 버킷 이름 환경 변수 키.
        genai_client: Google GenAI 클라이언트.
        storage_client: Google Cloud Storage 클라이언트.
    """
    ...
    VEO_DEFAULT_PERSON_GENERATION = 'dont_allow'  # 인물 생성 비허용 (안전 설정)
    VEO_DEFAULT_ASPECT_RATIO = '16:9'

    GCS_BUCKET_NAME_ENV_VAR = 'VIDEO_GEN_GCS_BUCKET'
    SIGNED_URL_EXPIRATION_SECONDS = 3600 * 48  # 48시간
    SIGNER_SERVICE_ACCOUNT_EMAIL_ENV_VAR = 'SIGNER_SERVICE_ACCOUNT_EMAIL'

    def __init__(self) -> None:
        """VideoGenerationAgent 인스턴스를 초기화합니다.

        Google GenAI 클라이언트와 GCS 클라이언트를 초기화합니다.
        필수 환경 변수가 설정되지 않으면 예외가 발생합니다.

        Raises:
            Exception: GenAI 클라이언트 초기화 실패 시.
            Exception: GCS 버킷 미설정 또는 GCS 클라이언트 초기화 실패 시.
        """
        ...
        # GCS 클라이언트 초기화 (Application Default Credentials 사용)
        try:
    ...
    async def _generate_signed_url(
        ...
        Args:
            blob_name: GCS 객체 이름 (경로).
            bucket_name: GCS 버킷 이름.
            expiration_seconds: URL 만료 시간 (초).

        Returns:
            str: 서명된 URL. 실패 시 gs:// URI 반환.
        """
    ...
    async def stream(
        ...
        """비디오 생성을 스트리밍 방식으로 처리합니다.

        진행률 업데이트와 최종 비디오 URL을 yield합니다.
        VEO 작업을 시작하고 완료될 때까지 폴링합니다.

        Args:
            prompt: 비디오 생성을 위한 텍스트 프롬프트.
            session_id: A2A 태스크 ID. 로깅과 고유 이름 지정에 사용됩니다.

        Yields:
            dict: 진행률 업데이트 또는 최종 결과를 포함하는 딕셔너리.
                - is_task_complete (bool): 태스크 완료 여부
                - updates (str): 진행 상태 메시지
                - progress_percent (int): 진행률 (0-100)
                - file_part_data (dict): 완료 시 비디오 파일 정보

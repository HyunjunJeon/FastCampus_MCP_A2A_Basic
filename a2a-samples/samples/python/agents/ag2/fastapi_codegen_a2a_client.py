"""FastAPI 코드 생성기 A2A 클라이언트.

이 모듈은 AG2의 A2aRemoteAgent를 사용하여 원격 리뷰어 에이전트와
통신하면서 FastAPI 애플리케이션을 생성하는 클라이언트를 구현합니다.

워크플로우:
    1. 로컬 CodeGenAgent가 FastAPI 애플리케이션 코드 생성
...
실행 방법:
    ...
    2. 클라이언트 실행: python fastapi_codegen_a2a_client.py

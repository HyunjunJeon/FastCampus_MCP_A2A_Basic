...
"""Airbnb 에이전트 핵심 구현 모듈.

이 모듈은 LangGraph ReAct 에이전트를 사용하여 Airbnb 숙소 검색을
수행하는 에이전트를 구현합니다.

주요 기능:
    - MCP 도구를 통한 Airbnb 검색
    - Gemini 모델 기반 자연어 처리
    - 스트리밍 응답 지원
    - 세션 상태 관리
...
class AirbnbAgent:
    ...
    MCP 도구와 LangGraph ReAct 에이전트를 사용하여
    Airbnb 숙소 검색 요청을 처리합니다.

    Note:
        SYSTEM_INSTRUCTION은 영문으로 유지됩니다 (LLM 지시사항).
    """
    ...
    def __init__(self, mcp_tools: list[Any]):
        """AirbnbAgent 인스턴스를 초기화합니다.

        Args:
            mcp_tools: 사전 로드된 MCP(Model Context Protocol) 도구 목록.
        """

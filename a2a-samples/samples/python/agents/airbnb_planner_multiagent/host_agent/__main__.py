"""호스트 에이전트 Gradio UI 진입점.

이 모듈은 날씨와 Airbnb 숙소 검색을 지원하는 멀티에이전트
시스템의 Gradio 웹 인터페이스를 실행합니다.

아키텍처:
    - Google ADK Runner를 사용한 라우팅 에이전트 실행
    - Gradio ChatInterface로 대화형 UI 제공
    - 원격 에이전트(Weather, Airbnb)와 A2A 통신
